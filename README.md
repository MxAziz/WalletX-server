...
**Add/Withdraw/Send Money Request:**
...
1. **Clone the repository**:
   ...
   ```bash
   git clone https://github.com/MxAziz/WalletX-server.git
   cd WalletX-server
   ```
...
| `BCRYPT_SALT_ROUND`         | Salt rounds for bcrypt       | `10`          |
| `JWT_ACCESS_TOKEN_SECRET`   | Access token secret key      | Required      |
| `JWT_ACCESS_TOKEN_EXPIRES`  | Access token expiry duration | `1d`         |
| `JWT_REFRESH_TOKEN_SECRET`  | Refresh token secret key     | Required      |
| `JWT_REFRESH_TOKEN_EXPIRES` | Refresh token expiry         | `30d`        |
